...
export default function NumPad({
  ...
  return (
    <div className="grid grid-cols-4 gap-2 p-3 bg-zinc-100 dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800">
      {/* Row 1: 7 8 9 ⌫ */}
      {['7', '8', '9'].map((d) => (
        <button key={d} className={digitBtn} onClick={() => { hapticLight(); onInput(d); }}>
          {d}
        </button>
      ))}
      <button
      ...
      <button
        ...
        onClick={() => { hapticLight(); onToggleSign(); }}
      >
        ±
      </button>
      ...
      {showConfirm && onConfirm ? (
        <button
          ...
          onClick={() => { if (!confirmDisabled) { hapticLight(); onConfirm(); } }}
