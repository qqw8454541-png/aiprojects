  ...
  persist(
    (set, get) => ({
      ...
      archiveSession: async () => {
        ...
        // Convert in-memory RoundResult[] → DbSessionRound[]
        const sessionRounds = completedRounds.map((r) => ({
        ...
        // Convert in-memory Player[] → DbSessionPlayer[]
        const sessionPlayers = state.players.map((p, i) => ({
          id: '',
          session_id: '',
          player_id: p.id,
          player_name: p.name,
          avatar_seed: p.avatarSeed,
          saved_member_id: p.savedMemberId,
          seat_index: i,
        }));
